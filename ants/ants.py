        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.container and self.ant.can_contain(insect):
                    ...
                    self.ant, self.entrance.ant = insect, insect
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                elif hasattr(self.ant, 'impostor') and not self.ant.impostor:
                    self.ant = self.ant
                else:
                    self.ant = None
            elif hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                if not hasattr(self.ant.ant, 'impostor'):
                    self.ant.ant = None
                elif hasattr(self.ant.ant, 'impostor') and self.ant.ant.impostor:
                    self.ant.ant = None
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while cur_place.name != hive.name:
            closest_bee = random_or_none(cur_place.bees)
            if closest_bee != None:
                return closest_bee
            else:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        """Return the nearest Bee in a Place that is not the HIVE, connected to
        the ThrowerAnt's Place by following entrances. Can only throw at the nearest Bee
        after at least 5 entrance transitions.

        This method returns None if there is no such Bee (or none in range)."""
        cur_place, change_places_counter = self.place, 0
        
        while change_places_counter < self.min_range:
            cur_place = cur_place.entrance
            change_places_counter += 1
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        """Return the nearest Bee in a Place that is not the HIVE, connected to
        the ThrowerAnt's Place by following entrances. Can only throw at a Bee at most 3
        ...
        while cur_place.name != hive.name:
            ...
            if closest_bee != None:
                return closest_bee
            elif change_places_counter < self.max_range:
                cur_place = cur_place.entrance
                change_places_counter += 1
            else:
                return None
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that does not lose armor when placed in water, 
        ...
        if self.impostor:
            ...
            while cur_place != None:
                if cur_place.ant != None:
                    if cur_place.ant not in self.buffed:
                        cur_place.ant.damage *= 2
                        self.buffed.append(cur_place.ant)
                    if cur_place.ant.container and cur_place.ant.ant != None and cur_place.ant.ant not in self.buffed:
                        cur_place.ant.ant.damage *= 2
                        self.buffed.append(cur_place.ant.ant)
